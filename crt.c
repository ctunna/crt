...
float lambert(vec3 *ray, vec3 *intersection, sphere_t s)
{
	float lam;
	vec3 norm;
...
vec3 send_ray(unsigned int depth, const vec3 *origin, vec3 *ray)
{
	int i;
	float t;
	vec3 pt, col, reflected, norm, new_dir;
	...
	for(i = 0; i < sphere_count; i++)
	{
		if(sphere_intersect(ray, &t, spheres[i])) {
			...
			col = spheres[i].color; 
			scale(&col, lambert(ray, &pt, spheres[i])); 
			break;
		}
	}
